...
用户让你“安装 pcbpilot”或在新机器上首次使用本仓库时：
...
2. 把脚本最后打印的**人工步骤**原样告诉用户：卸载旧 “PCB Pilot Connector” → 导入打印的 `.eext` →
   设置里打开“允许外部交互” → 重新加载编辑器。不得用 GUI 自动化代做。
3. 用户完成后运行 `pcbpilot health`，确认 `windows[]` 出现目标工程/文档且 `connectorVersion` 与
   `extension/extension.json` 一致；提醒用户重启 AI 客户端以加载 Skill 与 MCP。
...
本项目的现场 EDA 操作使用用户已打开并连接的宿主：桌面版或 Web 版、V3（3.2.x）或
V4 均可（pcbpilot 自有规则，2026-09-24 用户决定，不跟随上游的 V4-only 约定）。Agent 不自行
启动或切换宿主；报告必须注明宿主形态与精确版本，V3 与 V4 结果分别计为各自宿主线的回归。
