...
`easyeda pcb auto` 是离线引擎 `pkg/pcbauto` 的入口：读懂电路 → 按电压/电流定线宽与间距 →
决定层数与叠层 → 按机械约束和电压域布局 → 先平面扇出再拥塞协商布线 → 等长 → 独立 DRC 与
信号完整性检查。**它从不直接写编辑器**，只输出 `easyeda apply` 剧本，所有工程写入仍走
typed action，符合“禁止手工操作 EDA 工程”准则。

验证状态：`offline-verified`（5 块真实开源板 fixture 与合成隔离板离线回归；剧本通过
`easyeda apply --dry-run` 预检）。**尚未在用户的 EasyEDA 现场执行剧本并回读**，首次使用
...
1. **板子**：`--board board.json`（`easyeda pcb dump --include-copper --out board.json`），
...
- `zones`：手动指定电压域或功能块分区；不给时多电压域自动左右分区并留隔离带。
...
| `playbook.json` | `easyeda apply` 剧本：叠层 → 板框/孔/禁布区 → 器件位姿 → 走线/过孔 → 铺铜 → 翻转内电层 → 重铺 → 保存 → DRC |
...
- **布局**：多电压域左右分区 + 隔离禁铜带，隔离器件骑跨；功能块力导向初排，外围贴对应引脚，
  模拟退火优化加权线长（时钟/差分加权、地网为 0）、去耦距离、重叠、出区、禁布、限高；最后合法化。

## 执行

```bash
easyeda pcb dump --include-copper --out board.json --project <工程>
easyeda pcb auto run --board board.json --mech mech.json --power power.json --place --out-dir out/
# 先读 out/report.md 与 out/preview.svg，确认层数、电流、隔离与未布通项
easyeda apply out/playbook.json --project <工程> --dry-run
easyeda apply out/playbook.json --project <工程>
easyeda pcb save --project <工程>
easyeda doc reload --project <工程>
easyeda pcb drc --project <工程>
easyeda pcb check --project <工程>
```
