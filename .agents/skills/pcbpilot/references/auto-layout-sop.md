...
- 端口标签本体宽 11 raw，比 10 raw 脚距宽，所以同一条边上相邻的端口必须长短交错
  （已解布局都是 10 / 75–80 交替）。命名前的“边标签规划”对每条边上连续 ≥3 个、只有一个
  脚的端口岛整体做交错，并按两种相位、最短可行长档一次校验；规划挡住其余岛时自动退回
  原来的逐岛搜索。实测：AT32 BOOT0 下拉并入 QFN32 区从无解（320 万）变为 24 s 解出，
  LCD 20 万 → 5 万，L1 36 区其余不变、总耗时 −25%。
- 仍未解：晶振 / SWD 这类多脚外围接在 QFN 相邻脚上、同一边还有去耦电容时，电源符号无处可放
  ——需要协调摆放（参数化模板，下一步）；在此之前这类外围保持独立功能区。
