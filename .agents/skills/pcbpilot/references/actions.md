...
| `sch groups --pages <p1,p2> --out groups.json`，`schematic.rectangles.list` | 只读：逐页读矩形框（`y` 为原始 TopLeftY，CLI 统一翻转）、文本、器件 bbox；器件归入包含其 bbox 中心的**最小**框，框名取左上角最近文本；少于 `--min-members` 件或装下约整页的框（图框）忽略，未入框器件列在 `unframed`。输出即 `pcb auto run --groups` 的分组文件，任何原理图都适用。需连接器 ≥ 0.2.9 |
